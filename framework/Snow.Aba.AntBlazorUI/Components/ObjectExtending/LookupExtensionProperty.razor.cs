...
namespace Snow.Aba.AntdBlazorUI.Components.ObjectExtending
{
        ...
        where TEntity : IHasExtraProperties
    {
        ...
        protected virtual Task SelectedValueChanged(AutoCompleteOption selectedItem)
        {
        ...
        protected async Task SearchFilterChangedAsync(ChangeEventArgs args)
        {
            lookupItems = await GetLookupItemsAsync(args.Value.ToString());
        }
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            ...
            if (firstRender)
            {
                await SearchFilterChangedAsync(new ChangeEventArgs() { Value = string.Empty });
            }
        }
    }
}